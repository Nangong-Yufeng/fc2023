...
def preflight_command(the_connection, wp_home):
    ...
    # 设置home点
    while input("输入Y以设置home点：") == 'Y' and not set_home(the_connection, wp_home):
        continue

    # 设置模式为手动
    while input("输入Y以设置模式为手动：") == 'Y' and not mode_set(the_connection, 0):
        continue

    # 解锁
    while not arm(the_connection) and input("解锁失败，输入Y以解锁：") == 'Y':
        continue

    # 强制解锁
    if input("若解锁失败，可以输入Y以强制解锁：") == 'Y':
        while True:
            if force_arm(the_connection):
                break
